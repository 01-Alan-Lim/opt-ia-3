...
const STAGE = 2;
const ARTIFACT_TYPE = "foda_wizard_state";
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (error) return fail(500, "DB_ERROR", "No se pudo leer el estado FODA.", error);

    if (!data) return NextResponse.json({ ok: true, exists: false }, { status: 200 });

    return NextResponse.json(
      { ok: true, exists: true, chatId: data.chat_id ?? null, state: data.payload ?? null, updatedAt: data.updated_at ?? null },
      { status: 200 }
    );
  } catch (e: any) {
