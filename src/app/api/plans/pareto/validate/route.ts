  ...
  try {
    ...
    try {
      ...
      if (!evaluation || typeof evaluation.total_score !== "number") {
        console.error("[plans] pareto/validate: evaluación IA sin JSON válido", llmText);
        evalWarning = { warning: "Etapa 5 validada, pero la IA no devolvió un JSON válido." };
        evaluation = null;
      }
    } catch {
    ...
    console.error("[plans] pareto/validate: error interno", err);
